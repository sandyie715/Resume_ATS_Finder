...
def similarity_endpoint(jd: JDInput, resume: ResumeInput):
    ...
    try:
        ...
        # Run pipeline safely
        pipeline_result = run_similarity_pipeline(jd.dict() if jd else {}, resume_data) or {}

        # -----------------------------
        # 🔒 Ensure ALL expected fields exist
        # -----------------------------
        pipeline_result.setdefault("overall_similarity", 0.0)
        pipeline_result.setdefault("skills_similarity", 0.0)
        pipeline_result.setdefault("experience_similarity", 0.0)
        pipeline_result.setdefault("missing_keywords", [])
        pipeline_result.setdefault("experience_match", [])
        ...
        for item in experience_match:
            try:
                ...
                item["required"] = float(item.get("required", 0.0))
                item["candidate"] = float(item.get("candidate", 0.0))
                item["status"] = str(item.get("status", "insufficient"))
            except Exception as e:
                logger.warning(f"[Experience Match] Invalid entry {item}: {e}")
                item.update({"skill": "", "required": 0.0, "candidate": 0.0, "status": "insufficient"})
        ...
        # -----------------------------
        try:
            pipeline_result["experience_similarity"] = float(pipeline_result.get("experience_similarity", 0.0))
        except Exception as e:
        ...
        # ✅ Always return a valid Pydantic model
        return SimilarityResponse(**pipeline_result)
